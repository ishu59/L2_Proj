...
void WaitingVehicles::permitEntryToFirstInQueue() {
  ...
  //  auto cur_vehicle = _vehicles.front();
  //  auto cur_promise = std::move(_promises.front());
  if (getSize()) {
...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle) {
  ...
  std::promise<void> drive_promise;
  std::future<void> drive_ftr = drive_promise.get_future();
  _waitingVehicles.pushBack(vehicle, std::move(drive_promise));
  drive_ftr.get();
